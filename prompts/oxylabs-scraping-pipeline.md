# biasly Oxylabs 抓取流水线（手动抓取，session 9 / session 11 / session 12 / session 13 / session 16）

## 1. 目标

实现 AGENTS.md session 9 的 **scrape-to-insert 抓取入库流水线**（手动抓取路径，session 16），并让调度器（session 18）后续可以复用同一套流水线逻辑：
...
本任务**不做**：Oxylabs 调度器、/runs 结果处理、Vercel Cron、AI 分析（session 18/session 19）、pgvector（session 20）。这些由后续任务实现，但会复用本任务产出的流水线模块。
...
- `.env.local`：已配置 `OXY_WSA_USERNAME` / `OXY_WSA_PASSWORD`；**缺少 `BIASLY_ADMIN_SECRET`**（现有 `x-biasly-admin-secret` 是请求头名，不是环境变量名，AGENTS.md session 15/session 21 要求 `BIASLY_ADMIN_SECRET`）——测试前需用户补充

## 4. 决策或假设

1. **手动抓取路由**：`POST /api/scrape`，要求 `x-biasly-admin-secret` 请求头（session 15/session 16）。请求体可选 `{ sourceIds?: string[], limitPerSource?: number }`；缺省 = 所有活跃源、每源最多 5 篇（session 16 默认）
...
5. **拒绝策略从严**：non-article reject list（session 9）用路径段/子串匹配；URL 模式可疑但不确定时直接拒绝（session 12 "宁严勿宽"）
6. **详情验证（session 13）**：必须有文章特定 URL、特定标题、图片 URL、发布日期、有意义的正文；正文通过 **3+ 有意义的段落** 或 **清洗后 ≥900 有意义的字符** 即合格；清洗时移除 script/style/广告/通讯订阅/相关文章/热门/分享文本等
7. **slug 生成**：由标题生成 ASCII slug（小写、去符号、空格转连字符、截断 ~80 字符）；与已有 `original_url` / `canonical_url` / `slug` 唯一冲突时跳过或追加短随机后缀，不覆盖已有行（session 10 仅追加）
8. **并发**：首页抓取并发 3；详情抓取并发 5，避免触发站点/API 限流；每次 Oxylabs 请求设合理超时（如 120s）
9. **日志**：控制台日志（session 9 run logging）+ 写入 `logs` 表（service-role）；最终返回摘要对象
10. **不新增 API 路由**：除 `POST /api/scrape` 外本任务不建其他路由（调度器路由后续任务实现）
11. 引入 `cheerio` 与 `zod` 作为直接依赖（tech stack session 6 指定；zod 本次仅用于请求体校验）
...
- `lib/scraper/extract-homepage.ts` —— 首页候选文章链接提取（session 11）
- `lib/scraper/extract-article.ts` —— 详情页解析：标题/规范 URL/图片/日期/正文（JSON-LD + meta + DOM 兜底）
- `lib/scraper/urls.ts` —— URL 规范化、同源判断、候选 URL 过滤（session 12）、slug 生成
- `lib/scraper/validate.ts` —— article content gate（session 13）与正文清洗
- `lib/scraper/store.ts` —— 分批 URL 存在性检查（≤15/批）、仅追加插入、日志表写入
- `lib/scraper/pipeline.ts` —— session 9 流水线编排：源加载 → 首页 → 候选 → 去重 → 详情 → 验证 → 插入 → 摘要
- `lib/scraper/log.ts` —— 控制台 + `logs` 表双写日志
- `lib/scraper/admin.ts` —— `x-biasly-admin-secret` 校验（session 15），供 `/api/scrape` 使用（后续 `/api/analyze` 等复用）
...
- non-article reject list 作为只读常量（session 9 定义，集中维护）

### 6.3 Oxylabs 客户端（lib/scraper/oxylabs.ts）

- `fetchHomepageHtml(url: string): Promise<{ html: string; statusCode: number }>`
- 环境变量缺失时抛出明确错误；不打印凭据
- 响应非 2xx / 缺 `results[0]` / `status_code !== 200` 时抛出带上下文的错误（session 9 来源级错误）
...
- `looksLikeArticleUrl(url): boolean`：路径结构检查（session 12 从严）——通过条件（满足任一）：
  - 路径含日期（`20xx/xx/xx`）
  - 路径含 6+ 位数字 ID
...
- 清洗：移除 script/style/noscript/iframe/form/nav/footer/广告占位符/通讯订阅块/相关文章块/热门阅读块/"加载更多"/分享文本等（session 13），按 DOM 区块保留段落，段落少于 3 个大块时按句子边界拆分

### 6.7 验证与清洗（lib/scraper/validate.ts）

- `validateArticle(parsed, source): { ok: true } | { ok: false; reason: RejectionReason }`
- 标题过泛（如 "Home"、"News"、"Latest"、栏目名等）拒绝；正文按session 13 双标准通过；图片/日期必须存在
...
按session 9 顺序执行，每步控制台 + logs 表记录：

1. 加载活跃源（service-role，`active = true`；支持 `sourceIds` 过滤）
2. 并发抓取首页（session 6.3）；每源记录首页已获取/失败
3. 提取候选 → 过滤（reject list + URL 检查）→ 统计拒绝原因
4. 分批存在性检查（session 6.8），跳过重复
5. 并发抓取详情（session 6.6）→ 验证（session 6.7）
6. 仅追加插入（session 6.8）；每源达到 `limitPerSource` 后停止该源；
   详情抓取上限 = `limitPerSource × 3`（避免抓全量候选浪费 Oxylabs 配额并触发 429）
7. 输出最终摘要对象（session 9 字段清单）
...
- 仅 `POST`（session 14）；校验 `x-biasly-admin-secret` === `process.env.BIASLY_ADMIN_SECRET`，失败返回 401
- zod 校验请求体：`sourceIds?: string[]`、`limitPerSource?: number`（≥1，默认 5）
- 返回 `Response.json(summary)`；内部异常时返回 500 + 摘要（含错误信息，不泄露密钥）
- 路由处理程序保持薄层，逻辑全部在 `lib/scraper/*`（session 5 架构）

## 7. 安全要求

- `OXY_WSA_USERNAME` / `OXY_WSA_PASSWORD` / `BIASLY_ADMIN_SECRET` / `SUPABASE_SERVICE_ROLE_KEY` 仅服务端；不得出现在浏览器代码、日志或响应中
- 不将密钥放入 URL 查询串
- 抓取/分析/调度逻辑不进入客户端代码（session 21）
- 新增代码均为服务端模块（默认服务端组件/路由边界；必要时加 `server-only`）
- 不覆盖已有文章行；不删除/重置文章（session 10 仅追加）

## 8. 验收标准

- `POST /api/scrape`（带正确 admin 密钥）按session 9 全流程运行：控制台有整洁的逐步日志，响应含完整摘要对象
